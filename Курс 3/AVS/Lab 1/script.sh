...
echo -e "Mem free    $(free --mega | grep Mem | tr -s ' ' | cut -d " " -f4) MB"
echo -e "Mem total   $(free --mega | grep Mem | tr -s ' ' | cut -d " " -f2) MB"
echo -e "Mem used    $(free --mega | grep Mem | tr -s ' ' | cut -d " " -f3) MB\n"

echo -e "User        $(whoami)"
echo -e "IP          $(ip -br a show | grep UP | tr -s ' ' | cut -d " " -f3)"
echo -e "MAC address $(ip a | grep ether | tail -n 1 | tr -s ' ' | cut -d " " -f3)\n"
